...
export class Search_question_service {
  ...
  async filter_by_keyword(
    questions: Question[],
    keyword: string,
  ): Promise<Question[]> {
    return this.manager.transaction(async (manager) => {
      let answer: Question[];
      const quest = await this.manager.find(Keyword_Finder, {
        keyword: keyword,
      });
      let ids: Set<number>;
  ...
  async filter_by_date(questions: Question[], from_date: Date, to_date: Date) {
    ...
    for (let i = 0; i < questions.length; i++) {
      if (
        questions[i]['asked_on'] >= from_date &&
        questions[i]['asked_on'] <= to_date
      ) {
        answer.push(questions[i]);
      }
    }
  ...
  async search_by_keyword(keyword_s: string): Promise<Question[]> {
    return this.manager.transaction(async (manager) => {
      const questions = await this.manager.find(Keyword_Finder, {
        keyword: keyword_s,
      });
      const answer = [];
      for (let i = 0; i < questions.length; i++) {
        const qu = await this.manager.findOne(Question, {
  ...
  async questions_per_keyword(keywords: Keyword[]) {
    return this.manager.transaction(async (manager) => {
      ...
      for (let i = 0; i < keywords.length; i++) {
        const count = await this.manager.find(Keyword_Finder, {
